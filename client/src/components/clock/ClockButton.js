...
class ClockButton extends Component {
  ...
  clockIn = () => {
    const { clock, shift1ID, shift1clockIn, shift2ID, shift2clockIn, TonightShiftID, TonightShiftClockIn } = this.props
    // Mark clock time as now
    var clockTime = new Date()
    // if no clock time for this shift and the shift exists
    if (!shift1clockIn) {
      if (shift1ID) {
        clock(shift1clockIn, clockTime, 'start', shift1ID)
      }
      // else, if no clock time for this shift and the shift exists
    } else if (!shift2clockIn) {
      if (shift2ID) {
        clock(shift2clockIn, clockTime, 'start', shift2ID)
      }
      // else, if no clock time for this shift and the shift exists
    } else if (!TonightShiftClockIn) {
      if (TonightShiftID) {
        clock(TonightShiftClockIn, clockTime, 'start', TonightShiftID)
  ...
  clockOut = () => {
    const { clock, LastNightShiftID, LastNightShiftClockOut, shift1ID, shift1clockOut, shift2ID, shift2clockOut } = this.props
    // Mark clock time as now
    var clockTime = new Date()
    // if no clock time for this shift and the shift exists
    if (!LastNightShiftClockOut) {
      if (LastNightShiftID) {
        clock(LastNightShiftClockOut, clockTime, 'finish', LastNightShiftID)
      }
      // else, if no clock time for this shift and the shift exists
    } else if (!shift1clockOut) {
      if (shift1ID) {
        clock(shift1clockOut, clockTime, 'finish', shift1ID)
      }
      // else, if no clock time for this shift and the shift exists
    } else if (!shift2clockOut) {
      if (shift2ID) {
        clock(shift2clockOut, clockTime, 'finish', shift2ID)
      }
    }
    // change clock status
